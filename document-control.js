...
function startDocumentListeners(){
  ...
  unsubscribeDocuments = onSnapshot(query(collection(db, "documents"), orderBy("documentName")),
    snap => {
    ...
    err => {
      ...
      showLoadError(
        err.code === "permission-denied"
          ? "Permission denied reading documents. Confirm that the updated firestore.rules are deployed."
          : "Failed to load documents: " + err.message
      );
    }
  );

  unsubscribeHistory = onSnapshot(collection(db, "documentVersionHistory"),
    snap => {
      state.hist = {};
      snap.docs.forEach(d => {
...
onAuthStateChanged(auth, user => {
  ...
  if (user) {
    ...
    state.docs = [];
    state.hist = {};
    showLoadError("Sign in to load documents.");
  }
});
...
function render(){
  ...
  body.innerHTML = list.map(d => {
    ...
    const _href = normalizeUrl(d.sharedUrl); const link = _href ? `<a href="${esc(_href)}" target="_blank" rel="noopener noreferrer" class="btn sm primary" data-open="${esc(_href)}">Open</a>` : `<span class="muted">—</span>`;
...
function openForm(rec){
  ...
  overlay.querySelector("#dcForm").addEventListener("submit", async e => {
    ...
    const _nurl = normalizeUrl(url); try { new URL(_nurl); } catch { alert("Please enter a valid Shared Document URL"); return; }
