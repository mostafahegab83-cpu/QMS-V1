...
(function () {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      // Show signed-in user on the Home page (retry until DOM elements exist)
      (function setHomeBadge(tries){
        const he = document.getElementById("homeUserEmail");
        const hr = document.getElementById("homeUserRole");
        if (he && hr) {
          he.textContent = email || (user && user.email) || "(unknown)";
          hr.textContent = admin ? "Admin" : (role || "User");
        } else if ((tries||0) < 20) {
          setTimeout(() => setHomeBadge((tries||0)+1), 100);
        }
      })(0);
      ...
      window.__session = { user: null, email: null, isAdmin: false };
      const _he = document.getElementById("homeUserEmail");
      const _hr = document.getElementById("homeUserRole");
      if (_he) _he.textContent = "—";
      if (_hr) _hr.textContent = "";
