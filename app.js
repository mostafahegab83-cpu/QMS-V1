...
(function () {
  ...
  /* ---------- Records table ---------- */
  const REC_FILTER_FIELDS = ["auditId","auditDate","userName","processName","processPhase","dayWeek","department","subDepartment"];
  let _recFiltersWired = false;
  function wireRecordFilters() {
    if (_recFiltersWired) return;
    REC_FILTER_FIELDS.forEach(f => {
      const el = document.getElementById("recFilter_" + f);
      if (el) el.addEventListener("input", renderRecords);
    });
    const clr = document.getElementById("recFiltersClear");
    ...
    _recFiltersWired = true;
  }
  function getRecordFilterValues() {
    const out = {};
    REC_FILTER_FIELDS.forEach(f => {
      const el = document.getElementById("recFilter_" + f);
      out[f] = el ? el.value.trim().toLowerCase() : "";
    });
    return out;
  }
  function applyRecordFilters(rows) {
    const fv = getRecordFilterValues();
    return rows.filter(r => REC_FILTER_FIELDS.every(f => {
      if (!fv[f]) return true;
      const v = (r[f] == null ? "" : String(r[f])).toLowerCase();
      return v.includes(fv[f]);
    }));
  }
  ...
  function renderRecords() {
    ...
    if (!filtered.length) {
      tbody.innerHTML = `<tr><td colspan="${colCount}" style="text-align:center;color:var(--muted);padding:24px">No records match the current filters.</td></tr>`;
      return;
    }
...
(function () {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      const configAdmin = ADMIN_EMAILS.map(e => e.toLowerCase()).includes(email);

      // Approval check: configured admins are always approved; other users must be active in approved_users.
      let approved = configAdmin;
      let role = configAdmin ? "admin" : "user";
      if (!configAdmin) {
        try {
          const snap = await getDoc(doc(db, "approved_users", email));
          if (snap.exists()) {
            const data = snap.data() || {};
            approved = data.approved === true && data.revoked !== true;
            role = data.role || "user";
          }
        } catch (e) { approved = false; }
      }
      const admin = configAdmin || (approved && ["admin", "super_admin"].includes(String(role).toLowerCase()));
      ...
      window.__session = { user, email, isAdmin: admin, role };
...
(function () {
  ...
  async function approveUser(email) {
    ...
    if (!email) return;
    if (!window.__session?.isAdmin) { alert("Only admins can approve users."); return; }
    try {
      ...
      if (inp) inp.value = "";
      alert(`${email} is approved. If she still cannot log in, confirm she also exists in Firebase Authentication > Users.`);
    } catch (e) {
      console.error(e);
      alert("Failed: Missing permission to manage users. Deploy the updated firestore.rules file and make sure mostafa.hegab83@gmail.com exists in the super_admins collection.");
    }
  }
  async function revokeUser(email) {
    if (!confirm(`Revoke access for ${email}?`)) return;
    if (!window.__session?.isAdmin) { alert("Only admins can revoke users."); return; }
    try {
      await setDoc(doc(db, "approved_users", email), {
        email,
        approved: false,
